...
function causalTone(kind: 'event' | 'plan' | 'approval' | 'execution'): string {
...
export function Agent({
  ...
  const safeSelectedStepIndex = Math.min(selectedStepIndex, Math.max(displayedSteps.length - 1, 0));
  const selectedStep = displayedSteps[safeSelectedStepIndex] ?? displayedSteps[displayedSteps.length - 1] ?? null;
  ...
  const workflowStages = [
    ...
    },
    {
      ...
      status: (currentEvent || exceptionCount > 0 || steps.length > 0 ? 'complete' : summary ? 'active' : 'pending') as StageStatus,
    ...
    },
    {
      ...
      status: (selectedPlan ? 'complete' : currentEvent || exceptionCount > 0 || steps.length > 0 ? 'active' : 'pending') as StageStatus,
      detail: selectedPlan
    ...
    },
    {
      ...
      status: (pendingApproval ? 'active' : executionComplete ? 'complete' : selectedPlan ? 'active' : 'pending') as StageStatus,
    ...
    },
    {
      ...
      status: (hasReflection ? 'complete' : executionComplete ? 'active' : 'pending') as StageStatus,
