    ...
    <div className="space-y-8 pb-12">
      ...
      <section className="space-y-4">
        ...
        <div className="space-y-6">
          {/* Selected Plan (Full Width Row) */}
          <div className="rounded-card border-[2px] border-nearBlack bg-pureWhite p-6 shadow-card relative overflow-hidden">
            ...
            <div className="flex flex-col lg:flex-row gap-8">
              {/* Left Column: Basic Info & Execution */}
              <div className="flex-1 lg:max-w-[40%] flex flex-col justify-between">
                <div>
                  <h3 className="text-[28px] font-black text-nearBlack leading-tight">
                ...
                <div className="mt-6 pt-6 border-t border-borderGray/30">
                  <button className="w-full rounded bg-rausch px-6 py-3 text-[14px] font-bold text-pureWhite shadow-md hover:bg-rausch/90 transition-all flex items-center justify-center gap-2">
                    <Zap size={16} /> Execute Recommended Plan
                  </button>
                </div>
              </div>

              {/* Right Column: Why it wins */}
              <div className="flex-1 border-t lg:border-t-0 lg:border-l border-borderGray/30 pt-6 lg:pt-0 lg:pl-8">
                <h4 className="text-[12px] font-black uppercase tracking-widest text-secondaryGray mb-4 flex items-center gap-2">
                  <Zap size={14} className="text-rausch" /> Why this plan wins
                </h4>
                
                <ul className="space-y-3 mb-5">
                  {plan.metadata?.strategy_rationale?.split(". ").filter((s: string) => s.trim().length > 0).slice(0, 3).map((bullet: string, i: number) => (
                    <li key={i} className="text-[14px] text-nearBlack flex items-start gap-3">
                      <span className="text-rausch mt-1 font-bold">•</span> {bullet}.
                    </li>
                  ))}
                  {!plan.metadata?.strategy_rationale && (
                    <li className="text-[14px] text-nearBlack flex items-start gap-3">
                      <span className="text-rausch mt-1 font-bold">•</span> Optimizes cost and service trade-offs effectively.
                    </li>
                  )}
                </ul>

                {/* Collapsible Reasoning */}
                <div className="mt-5 rounded-lg bg-lightSurface/50 p-1">
                  <button
                    onClick={() => setExpandedReasoning(!expandedReasoning)}
                    className="w-full flex items-center justify-between p-3 text-[12px] font-bold text-secondaryGray hover:text-nearBlack transition-colors rounded-md hover:bg-lightSurface"
                  >
                    <span>{expandedReasoning ? "Hide Planner Details" : "View Detailed Planner Reasoning"}</span>
                    {expandedReasoning ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                  </button>
                  
                  {expandedReasoning && (
                    <div className="px-4 pb-4 pt-1 text-[13px] leading-relaxed text-secondaryGray italic border-t border-borderGray/30 mt-1">
          ...
          {/* Alternatives (2-Column Grid Row) */}
          {alternatives.length > 0 && (
            <div className="pt-2">
              <h4 className="text-[12px] font-black uppercase tracking-widest text-secondaryGray mb-4">
      ...
      <div className={`grid grid-cols-1 gap-8 ${combinedRisks.length > 0 ? "lg:grid-cols-[1.4fr_1fr]" : ""}`}>
        {/* 3. EXECUTION */}
        <section className="space-y-4 flex flex-col">
          <div className="flex items-center gap-3">
            <div className="flex h-8 w-8 items-center justify-center rounded-full border-[2px] border-nearBlack text-[14px] font-black text-nearBlack shrink-0">
              3
            </div>
          ...
          <div className="rounded-card border border-borderGray bg-pureWhite shadow-sm flex flex-col overflow-hidden max-h-[400px]">
        ...
        {combinedRisks.length > 0 && (
          <section className="space-y-4 flex flex-col">
            ...
            <div className="space-y-3 overflow-y-auto custom-scrollbar max-h-[400px] pr-2 pb-2 -mr-2">
              {["Critical", "Warning", "Info"].map((severity) => {
                ...
                return (
                  <div key={severity} className="rounded-card border border-borderGray bg-pureWhite shadow-sm overflow-hidden shrink-0">
                    <button 
