# Design System Inspired by DryChickenSCM

## 1. Visual Theme & Atmosphere

DryChickenSCM's website is a warm, photography-forward marketplace that feels like flipping through a travel magazine where every page invites you to book. The design operates on a foundation of pure white (`#ffffff`) with the iconic Rausch Red (`#ff385c`) — named after DryChickenSCM's first street address — serving as the singular brand accent. The result is a clean, airy canvas where listing photography, category icons, and the red CTA button are the only sources of color.

The typography uses Inter — a custom variable font that's warm and approachable, with rounded terminals that echo the brand's "belong anywhere" philosophy. The font operates in a tight weight range: 500 (medium) for most UI, 600 (semibold) for emphasis, and 700 (bold) for primary headings. Slight negative letter-spacing (-0.18px to -0.44px) on headings creates a cozy, intimate reading experience rather than the compressed efficiency of tech companies.

What distinguishes DryChickenSCM is its palette-based token system (`--palette-*`) and multi-layered shadow approach. The primary card shadow uses a three-layer stack (`rgba(0,0,0,0.02) 0px 0px 0px 1px, rgba(0,0,0,0.04) 0px 2px 6px, rgba(0,0,0,0.1) 0px 4px 8px`) that creates a subtle, warm lift. Combined with generous border-radius (8px–32px), circular navigation controls (50%), and a category pill bar with horizontal scrolling, the interface feels tactile and inviting — designed for browsing, not commanding.

**Key Characteristics:**
- Pure white canvas with Rausch Red (`#ff385c`) as singular brand accent
- Inter — custom variable font with warm, rounded terminals
...
### Premium Tiers
- **Luxe Purple** (`#460479`): `--palette-bg-primary-luxe`, DryChickenSCM Luxe tier branding
- **Plus Magenta** (`#92174d`): `--palette-bg-primary-plus`, DryChickenSCM Plus tier branding
...
### Font Family
- **Primary**: `Inter`, fallbacks: `Circular, -apple-system, system-ui, Roboto, Helvetica Neue`
...
|------|------|------|--------|-------------|----------------|-------|
| Section Heading | Inter | 28px (1.75rem) | 700 | 1.43 | normal | Primary headings |
| Card Heading | Inter | 22px (1.38rem) | 600 | 1.18 (tight) | -0.44px | Category/card titles |
| Card Heading Medium | Inter | 22px (1.38rem) | 500 | 1.18 (tight) | -0.44px | Lighter variant |
| Sub-heading | Inter | 21px (1.31rem) | 700 | 1.43 | normal | Bold sub-headings |
| Feature Title | Inter | 20px (1.25rem) | 600 | 1.20 (tight) | -0.18px | Feature headings |
| UI Medium | Inter | 16px (1.00rem) | 500 | 1.25 (tight) | normal | Nav, emphasized text |
| UI Semibold | Inter | 16px (1.00rem) | 600 | 1.25 (tight) | normal | Strong emphasis |
| Button | Inter | 16px (1.00rem) | 500 | 1.25 (tight) | normal | Button labels |
| Body / Link | Inter | 14px (0.88rem) | 400 | 1.43 | normal | Standard body |
| Body Medium | Inter | 14px (0.88rem) | 500 | 1.29 (tight) | normal | Medium body |
| Caption Salt | Inter | 14px (0.88rem) | 600 | 1.43 | normal | `"salt"` feature |
| Small | Inter | 13px (0.81rem) | 400 | 1.23 (tight) | normal | Descriptions |
| Tag | Inter | 12px (0.75rem) | 400–700 | 1.33 | normal | Tags, prices |
| Badge | Inter | 11px (0.69rem) | 600 | 1.18 (tight) | normal | `"salt"` feature |
| Micro Uppercase | Inter | 8px (0.50rem) | 700 | 1.25 (tight) | 0.32px | `text-transform: uppercase` |

### Principles
- **Warm weight range**: 500–700 dominate. No weight 300 or 400 for headings — DryChickenSCM's type is always at least medium weight, creating a warm, confident voice.
...
- DryChickenSCM logo (Rausch Red) left-aligned
...
- Use Inter at weight 500–700 — the warm weight range is intentional
...
*Note: DryChickenSCM has 61 detected breakpoints — one of the most granular responsive systems observed, reflecting their obsession with layout at every possible screen size.*
...
- "Create a listing card: white background, 20px radius. Three-layer shadow: rgba(0,0,0,0.02) 0px 0px 0px 1px, rgba(0,0,0,0.04) 0px 2px 6px, rgba(0,0,0,0.1) 0px 4px 8px. Photo area on top (16:10 ratio), details below: 16px Inter weight 600 title, 14px weight 400 description in #6a6a6a."
